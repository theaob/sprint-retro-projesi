...
function renderBoard(appEl, retro, user) {
  ...
  retro.columns.forEach(col => {
    ...
    col.entries.forEach(entry => {
      columnBodyMap[col.id].appendChild(createEntryCard(entry, retro.id));
    });

    bindColumnEvents(colEl, col, retro.id, columnBodyMap, authorName);
  });
  ...
  const socket = createRetroSocket(retro.id, {
    onEntryAdded(entry) {
      ...
      if (!document.getElementById(`entry-${entry.id}`)) {
        ...
        if (bodyEl) {
          bodyEl.appendChild(createEntryCard(entry, retro.id));
...
function bindColumnEvents(colEl, col, retroId, columnBodyMap, authorName) {
...
function createEntryCard(entry, retroId) {
