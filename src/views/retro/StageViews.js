...
export function SetupView({ columns, canFacilitate, busy, onStart, onEditColumns }) {
  return html`
    <section class="stage-card">
      ...
      <h2>${canFacilitate ? 'Retro hazırlanıyor' : 'Retro birazdan başlıyor'}</h2>
      <p>
        ${canFacilitate
          ? 'Sütunları kontrol et ve herkes geldiğinde yazma aşamasını başlat. Notlar oylamaya kadar sadece yazanına görünür.'
          : 'Kolaylaştırıcı yazma aşamasını başlatınca notlarını ekleyebileceksin. Notların oylamaya kadar sadece sana görünür.'}
      </p>
      <ul class="stage-card__lanes" aria-label="Sütunlar">
        ${columns.map((c, i) => html`<li class=${`lane-${i % 4}`}><i class="lane-dot"></i>${c.name}</li>`)}
      </ul>
      ${canFacilitate ? html`
        <div class="stage-card__actions">
          <${Button} variant="secondary" icon="columns" onClick=${onEditColumns}>Sütunları düzenle<//>
          <${Button} variant="primary" iconAfter="arrow-right" loading=${busy} onClick=${onStart}>Yazmaya başla<//>
        </div>
      ` : null}
    </section>
  `;
    ...
    <section class="discuss">
      ${focus ? html`
        <article class=${`focus-card lane-${focus.laneIndex % 4}`} aria-live="polite">
          <span class="focus-card__eyebrow"><i class="lane-dot"></i>Şu an konuşuluyor · ${focus.laneName}</span>
          <p class="focus-card__text">${focus.entry.text}</p>
          <div class="focus-card__foot">
            <span class="pill pill--vote"><${Icon} name="thumb" size=${16} /><span class="tabular">${focus.entry.votes ?? 0} oy</span></span>
          ...
          ${canFacilitate && nextUp ? html`
            <${Button} variant="primary" block iconAfter="arrow-right" onClick=${() => onFocus(nextUp.entry.id)}>Sıradaki not<//>
          ` : null}
        </article>
      ` : html`
        <div class="focus-card focus-card--empty">
          <p>${canFacilitate ? 'Konuşmak istediğin notu aşağıdan seç.' : 'Kolaylaştırıcı konuşulacak notu seçecek.'}</p>
        </div>
      `}

      ${queue.length > 0 ? html`
        <h2 class="section-title discuss__title">Sıradakiler</h2>
        <ol class="queue">
          ${queue.map(({ entry, laneIndex, laneName }, i) => html`
            <li class=${`queue__item lane-${laneIndex % 4}`}>
              ${canFacilitate ? html`
                <button type="button" class="queue__btn" onClick=${() => onFocus(entry.id)} aria-label=${`Bunu konuşalım: ${entry.text}`}>
  ...
  return html`
    <section class="summary">
      <div class="summary__head">
        ...
        <div>
          <h2>Retro tamamlandı</h2>
          <p class="summary__stats">
            <span><strong class="tabular">${noteCount}</strong> not</span>
            <span><strong class="tabular">${retro.voter_count || 0}</strong> kişi oy verdi</span>
            <span><strong class="tabular">${retro.columns.length}</strong> sütun</span>
          </p>
        </div>
      </div>
      <div class="summary__actions">
        <${Button} variant="primary" icon="download" onClick=${onExport}>Excel indir<//>
        ${canFacilitate ? html`<${Button} variant="secondary" icon="reopen" onClick=${onReopen}>Yeniden aç<//>` : null}
      </div>
      ${top.length > 0 ? html`
        <h3 class="section-title">En çok oy alanlar</h3>
        <ol class="queue">
